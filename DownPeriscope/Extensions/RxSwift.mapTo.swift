...
extension PrimitiveSequence where Trait == CompletableTrait {
    /**
     Returns a Single which complete event is mapped to constant provided as a parameter

     - parameter value: A constant that element of the input sequence is being replaced with
     - returns: A Single containing the value `value` provided as a parameter in case of completion
     */
    public func mapTo<Result>(_ value: Result) -> Single<Result> {
        return asObservable().mapTo(value).asSingle()
    }
}
